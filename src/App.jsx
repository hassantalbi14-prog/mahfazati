...
.iu:hover{border-color:rgba(255,255,255,.5);background:rgba(0,0,0,.25);}
...
export default function App(){
  ...
  const[budgetSettings,setBudgetSettings]=useState({
    ...
    allocations:[
      ...
      {id:4,name:"الاستثمار",icon:"📈",color:"#1a6b4a",pct:20,accountKeys:[],type:"investment"},
  ...
  const tl=t=>{const tp=t.type==="income"?"income":"expense";const c=gc(tp,t.catId);const s=gs(tp,t.catId,t.subId);return{cn:c?.name||"—",sn:s?.name||"",ic:c?.ci||c?.icon||"📌",hi:!!c?.ci,col:c?.color||"#888888"};};
  ...
  const resetData=()=>{
    ...
    setBudgetSettings({allocations:[
      ...
      {id:3,name:"الاستثمار",icon:"📈",color:"#1a6b4a",bg:"#d1fae5",accountKeys:[],fixed:false},
      ...
      <div style={{background:"white",borderRadius:24,padding:36,width:"100%",maxWidth:340,boxShadow:"0 8px 32px rgba(26,107,74,.12)",textAlign:"center"}}>
        ...
        <div style={{position:"relative",marginBottom:8}}>
          <input type={showPw?"text":"password"} placeholder="كلمة السر" value={pwInput}
            ...
            autoFocus/>
          <button onClick={()=>setShowPw(p=>!p)} style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#888888",display:"flex",alignItems:"center",padding:0}}>
        ...
        {resetStep===1&&<div style={{marginTop:12,padding:16,background:"#f0fdf4",borderRadius:12,border:"1px solid #10b98133",width:"100%",boxSizing:"border-box"}}>
          ...
          <button style={{width:"100%",marginTop:6,background:"none",border:"none",color:"#888888",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}
  ...
  const PeriodSelector=()=>{
    ...
    return(
      <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginBottom:8}}>
        <button onClick={()=>setPeriod(p=>({...p,type:"all"}))} style={{...S.btn(period.type==="all"?"#10b981":"#e8e8e4",false),padding:"6px 12px",fontSize:12,color:period.type==="all"?"white":"#666666"}}>الكل</button>
        <button onClick={()=>setPeriod(p=>({...p,type:"year"}))} style={{...S.btn(period.type==="year"?"#6366f1":"#e8e8e4",false),padding:"6px 12px",fontSize:12,color:period.type==="year"?"white":"#666666"}}>سنوي</button>
        <button onClick={()=>setPeriod(p=>({...p,type:"month"}))} style={{...S.btn(period.type==="month"?"#f59e0b":"#e8e8e4",false),padding:"6px 12px",fontSize:12,color:period.type==="month"?"white":"#666666"}}>شهري</button>
  ...
  const AccCard=({sec,icon,label,color,amount,count,children})=>(
    <div style={{...S.card,padding:0,overflow:"hidden",cursor:"pointer"}} onClick={()=>setOvExp(p=>({...p,[sec]:!p[sec]}))}>
      <div style={{display:"flex",alignItems:"center",gap:12,padding:"16px"}}>
        ...
        <div style={{flex:1}}><div style={{fontWeight:700,fontSize:15}}>{label}</div><div style={{fontSize:11,color:"#888888",marginTop:2}}>{count}</div></div>
        <div style={{fontSize:17,fontWeight:900,color,marginLeft:8}}>{fmt(amount)}</div>
        <div style={{color:"#888888",fontSize:22,transform:ovExp[sec]?"rotate(90deg)":"none",transition:"transform .2s"}}>›</div>
      </div>
  ...
  return (
    <div dir="rtl" style={{fontFamily:"'Tajawal',sans-serif",background:"#f5f5f0",minHeight:"100vh",color:"#1a1a1a",display:"flex",flexDirection:"column",position:"relative",overflow:"hidden",fontSize:(16*fontScale)+"px",zoom:fontScale}}>
      ...
      <div style={{padding:"20px 20px 0",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontSize:12,color:"#888888"}}>{new Date().toLocaleString("ar-MA",{month:"long",year:"numeric"})}</div><div style={{fontSize:22,fontWeight:900}}>محفظتي 💰</div></div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 20px 90px",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {page==="dashboard"&&<>
          <div style={{background:"linear-gradient(145deg,#1a6b4a,#0f4a33)",borderRadius:24,padding:26,position:"relative",overflow:"hidden",cursor:"pointer",boxShadow:"0 8px 32px rgba(26,107,74,.35)"}} onClick={()=>setPage("overview")}>
            ...
            <div style={{display:"flex",gap:12,marginTop:10,flexWrap:"wrap"}}>
              ...
              <div style={{textAlign:"center"}}><div style={{fontSize:10,color:"rgba(255,255,255,.7)"}}>الممتلكات</div><div style={{fontSize:13,fontWeight:700,color:"#1a1a1a"}}>{hideBalance?"•••":fmt(totAst)}</div></div>
              <div style={{width:1,background:"rgba(255,255,255,.3)"}}/>
              <div style={{textAlign:"center"}}><div style={{fontSize:10,color:"rgba(255,255,255,.7)"}}>السلف</div><div style={{fontSize:13,fontWeight:700,color:"#1a1a1a"}}>{hideBalance?"•••":fmt(totGiv-totOwd)}</div></div>
            </div>
          </div>
              ...
              <div style={{...S.card,cursor:"pointer"}} onClick={()=>setPage("budget")}>
                <div style={{...S.row,marginBottom:10}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <div><div style={{fontWeight:700,fontSize:14}}>ميزانية المصاريف</div><div style={{fontSize:11,color:"#888888"}}>إجمالي كلي</div></div>
                  </div>
            ...
            {txs.slice(0,5).map(t=>{const{cn,sn,ic,hi}=tl(t);return(
              <div key={t.id} className="tx">
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:11,color:"#888888"}}>{t.date}{sn&&` • ${sn}`}</div>
                </div>
        ...
        {page==="overview"&&(()=>{
          ...
          if(ovPage==="banks") return <>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:4}}>
              <button style={{...S.btn("#e8e8e4",false),padding:"8px 12px",fontSize:13,color:"#666666"}} onClick={()=>setOvExp(p=>({...p,ovPage:"main",ovBank:null}))}>← رجوع</button>
            ...
            {banks.map(b=>(
              <div key={b.id} style={{...S.card,cursor:"pointer"}} onClick={()=>setOvExp(p=>({...p,ovPage:"bank",ovBank:b.id}))}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <div style={{flex:1}}><div style={{fontWeight:700,fontSize:15}}>{b.name}</div><div style={{fontSize:11,color:"#888888"}}>{b.accounts.length} حساب</div></div>
            ...
            {banks.length===0&&<div style={{...S.card,textAlign:"center",padding:30,color:"#888888"}}>لا توجد بنوك</div>}
          </>;
          ...
          if(ovPage==="bank"&&ovBank){
            ...
            return <>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:4}}>
                <button style={{...S.btn("#e8e8e4",false),padding:"8px 12px",fontSize:13,color:"#666666"}} onClick={()=>setOvExp(p=>({...p,ovPage:"banks",ovBank:null}))}>← رجوع</button>
          ...
          if(ovPage==="cash") return <>
            ...
            {cash.length===0&&<div style={{...S.card,textAlign:"center",padding:30,color:"#888888"}}>لا توجد محافظ</div>}
          </>;
            ...
            return <>
              ...
              <div style={{...S.card,background:"#10b98110",border:"1px solid #10b98133",padding:14,marginBottom:8}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                  <span style={{fontSize:12,color:"#888888"}}>النوع: {inv.type||"استثمار"}</span>
                  <span style={{fontSize:12,color:"#888888"}}>{inv.date}</span>
                </div>
                <div style={{display:"flex",gap:8}}>
                  <div style={{flex:1,textAlign:"center"}}>
              ...
              {invTxs.length===0&&<div style={{...S.card,textAlign:"center",padding:20,color:"#888888"}}>ما كاينش معاملات مرتبطة</div>}
            </>;
          }
          ...
          if(ovPage==="invest") return <>
            ...
            {(()=>{
              ...
              return <>
                      ...
                      <div key={inv.id} style={{...S.card,padding:"14px 16px",cursor:"pointer"}} onClick={()=>setOvExp(p=>({...p,ovPage:"invDetail",ovInv:inv.id}))}>
                        <div style={{display:"flex",alignItems:"center",gap:12}}>
                          ...
                          <div style={{flex:1}}>
                            ...
                            <div style={{fontSize:11,color:"#888888"}}>{inv.type||"استثمار"} • {inv.date}</div>
                          </div>
                          <div style={{textAlign:"left"}}>
                ...
                {investments.length===0&&<div style={{...S.card,textAlign:"center",padding:30,color:"#888888"}}>ما كاينش استثمارات — ضغط + باش تزيد</div>}
              </>;
            })()}
          </>;
          ...
          return <>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <button style={{...S.btn("#e8e8e4",false),padding:"8px 12px",fontSize:13,color:"#666666"}} onClick={()=>setPage("dashboard")}>← رجوع</button>
            ...
            {[
              ...
              <div key={item.key} style={{...S.card,cursor:"pointer"}} onClick={()=>setOvExp(p=>({...p,ovPage:item.key,ovBank:null}))}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <div style={{flex:1}}><div style={{fontWeight:700,fontSize:16}}>{item.label}</div><div style={{fontSize:11,color:"#888888"}}>{item.sub}</div></div>
        ...
        {page==="settings"&&<>
          ...
          <div style={{...S.card,padding:0,overflow:"hidden"}}>
            <div style={{padding:"10px 16px 6px",fontSize:11,color:"#888888",fontWeight:700,letterSpacing:1,background:"#f8fafc",borderBottom:"1px solid #e2e8f0"}}>التصنيفات</div>
            {[{id:"expCat",icon:"🔴",label:"تصنيفات النفقات",count:`${cats.expense.length} تصنيف`},{id:"incCat",icon:"🟢",label:"تصنيفات الدخل",count:`${cats.income.length} تصنيف`}].map((item,i,arr)=>(
              <div key={item.id} style={{display:"flex",alignItems:"center",padding:"16px",cursor:"pointer",borderBottom:i<arr.length-1?"1px solid #e2e8f0":"none"}} onClick={()=>setDp(item.id)}>
                <div style={{width:42,height:42,borderRadius:12,background:"#f5f5f0",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,marginLeft:14,flexShrink:0}}>{item.icon}</div>
                <div style={{flex:1}}><div style={{fontSize:16,fontWeight:700,color:"#1a1a1a"}}>{item.label}</div><div style={{fontSize:12,color:"#888888"}}>{item.count}</div></div>
                <ChevronLeft size={18} color="#888888"/>
              </div>
            ))}
          </div>
          {/* ====== إعدادات الميزانية ====== */}
          <div style={{...S.card,padding:0,overflow:"hidden"}}>
            <div style={{padding:"10px 16px 6px",fontSize:11,color:"#888888",fontWeight:700,letterSpacing:1,background:"#f8fafc",borderBottom:"1px solid #e2e8f0"}}>⚙️ الميزانية الذكية</div>
            ...
            <div style={{borderBottom:"1px solid #f1f5f9"}}>
              <div style={{padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}} onClick={()=>setBudgetSec(p=>({...p,goals:!p.goals}))}>
                ...
                <span style={{fontSize:16,color:"#888888"}}>{budgetSec.goals?"▲":"▼"}</span>
              </div>
            {budgetSec.goals&&<div style={{padding:"0 16px 14px"}}>
              {[{key:"incomeGoal",autoKey:"incomeAuto",label:"💰 هدف الدخل",color:"#1a6b4a"},{key:"expenseGoal",autoKey:"expenseAuto",label:"💸 هدف المصاريف",color:"#ef4444"}].map(({key,autoKey,label,color})=>(
                <div key={key} style={{marginBottom:14}}>
                  <div style={{fontWeight:700,color,marginBottom:8,fontSize:13}}>{label}</div>
                  <div style={{display:"flex",gap:6,marginBottom:8}}>
                    <button style={{...S.btn(!(budgetSettings.goals?.[autoKey])?"#10b981":"#e8e8e4",false),flex:1,padding:"7px",fontSize:12,color:!(budgetSettings.goals?.[autoKey])?"white":"#666666"}}
                      onClick={()=>setBudgetSettings(p=>({...p,goals:{...(p.goals||{incomeGoal:15000,expenseGoal:5000}),[autoKey]:false}}))}>يدوي</button>
                    <button style={{...S.btn(budgetSettings.goals?.[autoKey]?"#10b981":"#e8e8e4",false),flex:1,padding:"7px",fontSize:12,color:budgetSettings.goals?.[autoKey]?"white":"#666666"}}
                      onClick={()=>setBudgetSettings(p=>({...p,goals:{...(p.goals||{incomeGoal:15000,expenseGoal:5000}),[autoKey]:true}}))}>تلقائي ✨</button>
                  </div>
                  {!budgetSettings.goals?.[autoKey]&&<input style={S.inp} type="number" value={budgetSettings.goals?.[key]||0}
                    onChange={e=>setBudgetSettings(p=>({...p,goals:{...(p.goals||{}),incomeGoal:p.goals?.incomeGoal||15000,expenseGoal:p.goals?.expenseGoal||5000,[key]:parseFloat(e.target.value)||0}}))}/>}
                  {budgetSettings.goals?.[autoKey]&&(()=>{
                    const allM=[...new Set(txs.map(t=>t.date.slice(0,7)))];
                    const avg=allM.length>0?txs.filter(t=>t.type===(key==="incomeGoal"?"income":"expense")&&!t.isTransfer).reduce((s,t)=>s+t.amount,0)/allM.length:0;
                    return <div style={{padding:"8px 12px",background:"#f8fafc",borderRadius:8,fontSize:12,color:"#666666"}}>متوسط: <span style={{color,fontWeight:700}}>{fmt(avg)}</span></div>;
                  })()}
                </div>
              ))}
            </div>}
            ...
            <div style={{borderBottom:"1px solid #f1f5f9"}}>
              <div style={{padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}} onClick={()=>setBudgetSec(p=>({...p,tranches:!p.tranches}))}>
                ...
                <span style={{fontSize:16,color:"#888888"}}>{budgetSec.tranches?"▲":"▼"}</span>
              </div>
            {budgetSec.tranches&&<div style={{padding:"0 16px 14px"}}>
              {(budgetSettings.tranches||[]).map(tr=>{
                ...
                return(
                  <div key={tr.id} style={{borderRadius:12,padding:14,border:`1px solid ${total===100?"#10b98133":"#ef444433"}`,marginBottom:10,background:total===100?"#f0fdf4":"#fff5f5"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                      <span style={{fontWeight:800,fontSize:14}}>شريحة {tr.id}</span>
                      <span style={{color:total===100?"#10b981":"#ef4444",fontWeight:700}}>{total}% {total===100?"✅":"⚠️"}</span>
                    </div>
                    <div style={{display:"flex",gap:6,marginBottom:10}}>
                      {[["min","من"],["max","إلى"],["fix","فيكس"]].map(([f,lbl])=>(
                        <div key={f} style={{flex:1}}>
                          <div style={{fontSize:10,color:"#888888",marginBottom:3}}>{lbl} (د.م)</div>
                          <input style={{...S.inp,padding:"6px 8px",fontSize:12}} type="number"
                            value={f==="max"&&tr.max===999999?"":tr[f]} placeholder={f==="max"?"∞":""}
                            onChange={e=>setBudgetSettings(p=>({...p,tranches:(p.tranches||[]).map(x=>x.id===tr.id?{...x,[f]:parseFloat(e.target.value)||(f==="max"?999999:0)}:x)}))}/>
                        </div>
                      ))}
                    </div>
                    {(budgetSettings.allocations||[]).map(a=>(
                      <div key={a.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                        <span style={{fontSize:18,width:24,flexShrink:0}}>{a.icon}</span>
                        <span style={{flex:1,fontSize:13}}>{a.name}</span>
                        <input style={{...S.inp,width:64,textAlign:"center",padding:"6px 8px",fontSize:14,fontWeight:700,color:a.color,border:`1px solid ${a.color}44`}}
                          type="number" min="0" max="100" value={tr.pcts[a.id]||0}
            ...
            <div style={{borderBottom:"1px solid #f1f5f9"}}>
              <div style={{padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}} onClick={()=>setBudgetSec(p=>({...p,alloc:!p.alloc}))}>
                ...
                <span style={{fontSize:16,color:"#888888"}}>{budgetSec.alloc?"▲":"▼"}</span>
              </div>
            {budgetSec.alloc&&<div style={{padding:"0 16px 14px"}}>
              {(budgetSettings.allocations||[]).map(a=>{
                ...
                const bal=allAcc.filter(ac=>(a.accountKeys||[]).includes(ac.key)).reduce((s,ac)=>s+(ac.balance||0),0);
                return(
                  <div key={a.id} style={{borderRadius:12,padding:12,border:`2px solid ${a.color}33`,marginBottom:10}}>
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                      <div style={{width:38,height:38,borderRadius:10,background:a.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>{a.icon}</div>
                      <div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{a.name}</div><div style={{fontSize:12,color:a.color,fontWeight:700}}>{fmt(bal)}</div></div>
                    </div>
                    {a.type==="expenses"&&(
                      <div style={{display:"flex",gap:6,marginBottom:8}}>
                        <div style={{flex:1}}><div style={{fontSize:10,color:"#888888",marginBottom:3}}>🔔 تنبيه (د.م)</div>
                        ...
                        <div style={{flex:1}}><div style={{fontSize:10,color:"#888888",marginBottom:3}}>💸 تحويل طوارئ</div>
                    ...
                    {(a.accountKeys||[]).map(key=>{
                      ...
                      return acc?(
                        <div key={key} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:a.color+"12",borderRadius:8,marginBottom:6}}>
          ...
          <div style={{...S.card,padding:0,overflow:"hidden"}}>
            <div style={{padding:"10px 16px 6px",fontSize:11,color:"#888888",fontWeight:700,letterSpacing:1,background:"#f8fafc",borderBottom:"1px solid #e2e8f0"}}>الحساب</div>
            ...
            <div style={{padding:"14px 16px",borderTop:"1px solid #f1f5f9"}}>
              ...
              <div style={{fontSize:11,color:"#888888",marginBottom:10}}>إيميل أو رقم الهاتف — كيبان ملي تنسى كلمة السر</div>
          ...
          if(ovExp.debtPage)return <>
            ...
            {ovExp.debtPage==="salaf"&&<>
              <div style={{display:"flex",gap:8}}>
                <div style={{flex:1,textAlign:"center",background:"#10b98110",borderRadius:12,padding:"12px 6px",border:"1px solid #10b98133"}}>
                  ...
                  <div style={{fontSize:11,color:"#888888"}}>{salafGiven.length} شخص</div>
                </div>
                <div style={{flex:1,textAlign:"center",background:"#ef444410",borderRadius:12,padding:"12px 6px",border:"1px solid #ef444433"}}>
                  ...
                  <div style={{fontSize:11,color:"#888888"}}>{salafTaken.length} شخص</div>
                </div>
              </div>
              <div style={{display:"flex",gap:8}}>
                ...
                <div key={l.id} style={S.card}>
                  <div style={{...S.row,marginBottom:8}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      <div><div style={{fontSize:14,fontWeight:700,color:"#1a1a1a"}}>{l.person}</div><div style={{fontSize:11,color:"#888888"}}>{l.date}{l.note?` · ${l.note}`:""}</div></div>
                    </div>
                    <div style={{textAlign:"left"}}>
                      <div style={{fontSize:16,fontWeight:900,color:l.kind==="أعطيت"?"#10b981":"#ef4444"}}>{fmt(l.remaining)}</div>
                      {l.remaining<l.amount&&<div style={{fontSize:10,color:"#888888"}}>من {fmt(l.amount)}</div>}
                    </div>
                  </div>
                  <div style={{display:"flex",gap:8}}>
              ...
              {salafGiven.length===0&&salafTaken.length===0&&<div style={{...S.card,textAlign:"center",padding:30,color:"#888888"}}>لا توجد سلف</div>}
            </>}
            {ovExp.debtPage==="qorudh"&&<>
              ...
              {qorudh.map(l=>{
                ...
                return(
                  <div key={l.id} style={S.card}>
                    <div style={{...S.row,marginBottom:10}}>
                      <div><div style={{fontSize:15,fontWeight:700,color:"#1a1a1a"}}>{l.person}</div><div style={{fontSize:12,color:"#888888"}}>فائدة {l.interest}%{l.inst?` · قسط ${fmt(l.minst)}/شهر`:""}</div></div>
                      <div style={{textAlign:"left"}}><div style={{fontSize:17,fontWeight:900,color:"#ef4444"}}>{fmt(l.remaining)}</div><div style={{fontSize:11,color:"#888888"}}>من {fmt(l.amount)}</div></div>
                    </div>
                    <div className="pbar" style={{marginBottom:6}}><div className="pfill" style={{width:pct+"%",background:"#10b981"}}/></div>
                    <div style={{...S.row,marginBottom:10}}>
                      <span style={{fontSize:11,color:"#888888"}}>مسدد {pct.toFixed(0)}%</span>
              ...
              {qorudh.length===0&&<div style={{...S.card,textAlign:"center",padding:30,color:"#888888"}}>لا توجد قروض</div>}
            </>}
            {ovExp.debtPage==="credit"&&<>
              <div style={{...S.card,textAlign:"center",background:"#f59e0b10",border:"1px solid #f59e0b33"}}>
                ...
                <div style={{fontSize:11,color:"#888888",marginTop:4}}>{creditTxs.length} معاملة</div>
              </div>
              {creditTxs.length===0?
                <div style={{...S.card,textAlign:"center",padding:30}}><div style={{fontSize:30,marginBottom:8}}>✅</div><div style={{color:"#888888"}}>ما كاين ديون كريدي</div></div>:
                creditTxs.map(t=>(
                  <div key={t.id} style={S.card}>
                    <div style={{...S.row,marginBottom:10}}>
                      <div><div style={{fontSize:14,fontWeight:700,color:"#1a1a1a"}}>{t.desc}</div><div style={{fontSize:11,color:"#888888"}}>{t.date}</div></div>
                    ...
                    {ovExp[`pay_${t.id}`]?
                      <div style={{display:"flex",flexDirection:"column",gap:8}}>
                        ...
                        <button style={{background:"none",border:"none",color:"#888888",fontFamily:"Tajawal",fontSize:12,cursor:"pointer"}} onClick={()=>setOvExp(p=>({...p,[`pay_${t.id}`]:false}))}>إلغاء</button>
                      </div>:
              ...
              <div key={item.key} style={{...S.card,cursor:"pointer",padding:0,overflow:"hidden"}} onClick={()=>setOvExp(p=>({...p,debtPage:item.key}))}>
                <div style={{display:"flex",alignItems:"center",padding:"16px"}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:12,color:"#888888",marginTop:2}}>{item.sub}</div>
          ...
          return <>
            ...
            <div style={{background:"linear-gradient(135deg,#0f172a,#1e293b)",borderRadius:20,padding:18,marginBottom:12,boxShadow:"0 4px 24px rgba(15,23,42,0.2)"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                ...
                {tranche&&<span style={{background:"#10b98120",color:"#1a6b4a",padding:"3px 10px",borderRadius:20,fontSize:10,fontWeight:700}}>
              ...
              return(
                <div key={a.id} onClick={()=>setSelBucket(selBucket===a.id?null:a.id)}
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{fontWeight:900,fontSize:15,color:"#1a1a1a"}}>{a.name}</div>
                    ...
                    <div style={{textAlign:"left"}}>
                      ...
                      <div style={{fontSize:10,color:"#888888",textAlign:"left"}}>خروج</div>
                    </div>
                  </div>
                  <div style={{height:6,background:"#f5f5f0",borderRadius:3,overflow:"hidden",marginBottom:8}}>
                  ...
                  {selBucket===a.id&&<div style={{marginTop:14,borderTop:"1px solid #f1f5f9",paddingTop:14}}>
                    ...
                    <div style={{background:"#f8fafc",borderRadius:14,border:"1.5px solid #e2e8f0",overflow:"hidden",marginBottom:10}}>
                      <div style={{padding:"10px 14px",fontSize:11,fontWeight:700,color:"#666666",borderBottom:"1px solid #e2e8f0",display:"flex",alignItems:"center",gap:6}}>
                        <div style={{width:8,height:8,borderRadius:"50%",background:a.color}}/>
                        📊 الميزانية
                      </div>
                      <div style={{padding:"10px 14px",display:"flex",justifyContent:"space-between"}}>
                        <span style={{fontSize:13,color:"#666666",fontWeight:600}}>📥 دخل من التقسيم</span>
                        <span style={{fontSize:15,fontWeight:900,color:"#1a6b4a"}}>+{fmt(allocated)}</span>
                      </div>
                      <div style={{height:1,background:"#f5f5f0",margin:"0 14px"}}/>
                      <div style={{padding:"10px 14px",display:"flex",justifyContent:"space-between"}}>
                        <span style={{fontSize:13,color:"#666666",fontWeight:600}}>📤 خروج</span>
                      ...
                      {noAcc?
                        ...
                        allAcc.filter(ac=>(a.accountKeys||[]).includes(ac.key)).map(ac=>(
                          <div key={ac.key} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",borderBottom:"1px solid #f1f5f9"}}>
                            ...
                            <div style={{flex:1}}>
                              <div style={{fontSize:13,fontWeight:700,color:"#1a1a1a"}}>{ac.name||ac.key}</div>
                              <div style={{display:"flex",gap:10,marginTop:3}}>
                                <span style={{fontSize:11,fontWeight:700,color:"#1a6b4a"}}>+{txs.filter(t=>t.type==="income"&&t.ref===ac.key&&!t.isTransfer&&!t.isLoan).reduce((s,t)=>s+t.amount,0).toLocaleString("ar-MA")}</span>
                            ...
                            <div>
                              <div style={{fontSize:16,fontWeight:900,color:"#1a1a1a",textAlign:"left"}}>{(ac.balance||0).toLocaleString("ar-MA")} د.م</div>
                              <div style={{fontSize:10,color:"#888888",textAlign:"left"}}>الرصيد</div>
                            </div>
                          </div>
                        ))
                      }
                    ...
                    <div style={{background:"#f8fafc",borderRadius:14,border:"1.5px solid #e2e8f0",overflow:"hidden"}}>
                      ...
                      {txs.filter(t=>(a.accountKeys||[]).includes(t.ref)&&!t.isTransfer).slice(0,5).length===0?
                        <div style={{padding:16,textAlign:"center",color:"#888888",fontSize:13}}>ما كاينش معاملات</div>:
                        txs.filter(t=>(a.accountKeys||[]).includes(t.ref)&&!t.isTransfer).slice(0,5).map(t=>(
                          <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderBottom:"1px solid #f1f5f9"}}>
                            ...
                            <div style={{flex:1}}>
                              <div style={{fontSize:13,fontWeight:700,color:"#1a1a1a"}}>{t.desc||"معاملة"}</div>
          ...
          if(repTab==="yearly"){
            ...
            return <>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                {selMonth&&<button style={{...S.btn("#e8e8e4",false),padding:"8px 12px",fontSize:13,color:"#666666"}} onClick={()=>setOvExp(p=>({...p,repMonth:null}))}>← رجوع</button>}
              </div>
              {!selMonth&&<>
                <div style={{...S.card,background:"#f8fafc"}}>
                  <div style={{fontSize:12,color:"#666666",marginBottom:10,fontWeight:700}}>📅 ملخص {selYear}</div>
                  <div style={{display:"flex",gap:8}}>
                    <div style={{flex:1,textAlign:"center",background:"#10b98110",borderRadius:10,padding:"10px 4px"}}><div style={{fontSize:10,color:"#1a6b4a"}}>الدخل</div><div style={{fontSize:14,fontWeight:900,color:"#1a6b4a"}}>{fmt(yInc)}</div></div>
                    ...
                    return(
                      <div key={md.m} style={{padding:"12px 0",borderBottom:"1px solid #e2e8f0",cursor:"pointer"}} onClick={()=>setOvExp(p=>({...p,repMonth:md.m}))}>
                        ...
                        {(md.inc>0||md.exp>0)&&<>
                          <div className="pbar"><div className="pfill" style={{width:pct+"%",background:color}}/></div>
                          <div style={{...S.row,marginTop:4}}>
                            <span style={{fontSize:10,color:"#888888"}}>دخل: {fmt(md.inc)}</span>
                            <span style={{fontSize:10,color:"#888888"}}>مصاريف: {fmt(md.exp)}</span>
                          </div>
                        </>}
                      </div>
                    );
              ...
              {selMonth&&(()=>{
                ...
                return <>
                  <div style={{...S.card,background:"#f8fafc"}}>
                    ...
                    <div style={{display:"flex",gap:8}}>
                      <div style={{flex:1,textAlign:"center",background:"#10b98110",borderRadius:10,padding:"8px 4px"}}><div style={{fontSize:10,color:"#1a6b4a"}}>الدخل</div><div style={{fontSize:14,fontWeight:900,color:"#1a6b4a"}}>{fmt(md.inc)}</div></div>
                  ...
                  {incTxs.length>0&&<div style={S.card}>
                    <div style={{fontWeight:700,marginBottom:10,color:"#1a6b4a"}}>💰 المداخل ({incTxs.length})</div>
                    {incTxs.map(t=>{const c=gc("income",t.catId);return(
                      <div key={t.id} className="tx">
                        ...
                        <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{t.desc||c?.name||"—"}</div><div style={{fontSize:10,color:"#888888"}}>{t.date}</div></div>
                        <span style={{fontSize:13,fontWeight:700,color:"#1a6b4a"}}>+{fmt(t.amount)}</span>
                      </div>
                    );})}
                  </div>}
            ...
            return <>
              ...
              <div style={{...S.card,padding:"12px 16px"}}>
                <div style={{display:"flex",gap:8,marginBottom:10}}>
                  <div style={{flex:1}}><div style={{fontSize:10,color:"#888888",marginBottom:4}}>من</div><input style={{...S.inp,padding:"8px 10px",fontSize:12}} type="date" value={dateFrom} onChange={e=>setOvExp(p=>({...p,fDateFrom:e.target.value}))}/></div>
                  <div style={{flex:1}}><div style={{fontSize:10,color:"#888888",marginBottom:4}}>إلى</div><input style={{...S.inp,padding:"8px 10px",fontSize:12}} type="date" value={dateTo} onChange={e=>setOvExp(p=>({...p,fDateTo:e.target.value}))}/></div>
                </div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  {[["all","كلشي","#10b981"],["income","مداخل","#10b981"],["expense","مصاريف","#ef4444"],["salaf","سلف","#8b5cf6"],["credit","كريدي","#f59e0b"]].map(([val,lbl,col])=>(
                    <button key={val} onClick={()=>setOvExp(p=>({...p,fCat:val}))}
                      style={{...S.btn(catFilter===val?col:"#e8e8e4",false),padding:"7px 12px",fontSize:12,color:catFilter===val?"white":"#666666"}}>{lbl}</button>
                  ))}
                </div>
              </div>
              <div style={{...S.card,textAlign:"center",background:"#10b98110",border:"1px solid #10b98133"}}>
                <div style={{fontSize:12,color:"#666666",fontWeight:700,marginBottom:4}}>إجمالي الفترة</div>
                <div style={{fontSize:26,fontWeight:900,color:"#1a6b4a"}}>{fmt(total)}</div>
                <div style={{fontSize:11,color:"#888888",marginTop:4}}>{filtTxs.length} معاملة</div>
              </div>
              <div style={S.card}>
                {filtTxs.slice(0,50).map(t=>{
                  ...
                  return(<div key={t.id} className="tx">
                    ...
                    <div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.desc||c?.name||"—"}</div><div style={{fontSize:10,color:"#888888"}}>{t.date}</div></div>
                ...
                {filtTxs.length===0&&<div style={{textAlign:"center",color:"#888888",padding:20}}>لا توجد بيانات</div>}
        ...
        <div className="mwp" onClick={cm}>
          <div className="mbx" onClick={e=>e.stopPropagation()}>
            <div style={{...S.row,marginBottom:20}}>
              ...
              <button onClick={cm} style={{background:"none",border:"none",color:"#666666",cursor:"pointer"}}><X size={20}/></button>
            </div>
            ...
            {(modal==="addMCat"||modal==="edMCat")&&<div style={S.col}>
              ...
              <div style={{fontSize:12,color:"#888888",marginBottom:4}}>الأيقونة:</div>
              <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                <div>
                  <div className="iu" style={{width:52,height:52}} onClick={()=>(modal==="addMCat"?iRef:eiRef).current.click()}>
                    {(modal==="addMCat"?form.ci:ei?.ci)?<img src={modal==="addMCat"?form.ci:ei.ci} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<><Camera size={16} color="#888888"/><div style={{fontSize:9,color:"#888888",marginTop:2}}>تحميل</div></>}
                  </div>
            ...
            {modal==="edSCat"&&ei&&<div style={S.col}>
              <div style={{padding:"10px 14px",background:"#f8fafc",borderRadius:10,fontSize:13,color:"#666666"}}>الحالي: <strong style={{color:"#1a1a1a"}}>{ei.name}</strong></div>
            ...
            {modal==="changePw"&&<div style={S.col}>
              ...
              <div style={{marginTop:12,borderTop:"1px solid rgba(255,255,255,.1)",paddingTop:12}}>
                <div style={{fontSize:12,color:"#888888",marginBottom:6}}>📱 جهة استرجاع كلمة السر</div>
                <input style={S.inp} type="text" placeholder="إيميل أو رقم الهاتف" value={recoveryContact}
                  onChange={e=>setRecoveryContact(e.target.value)}/>
                <div style={{fontSize:10,color:"#888888",marginTop:4}}>تستعمل للتذكير بكلمة السر عند النسيان</div>
              </div>
            </div>}
            ...
            {modal==="budgetSettings"&&<div style={S.col}>
              ...
              {/* الأهداف الشهرية */}
              <div style={{fontSize:12,color:"#888888",fontWeight:700}}>🎯 الأهداف الشهرية</div>
              <div style={{background:"white",borderRadius:12,padding:12,border:"1px solid #10b98133"}}>
                ...
                {budgetSettings.goals?.incomeAuto&&<div style={{fontSize:11,color:"#888888",padding:"8px 0"}}>يحسب تلقائياً من متوسط الدخل الشهري</div>}
                ...
                {budgetSettings.goals?.expenseAuto&&<div style={{fontSize:11,color:"#888888",padding:"8px 0"}}>يحسب تلقائياً من متوسط المصاريف الشهرية</div>}
              </div>

              {/* الشرائح */}
              <div style={{fontSize:12,color:"#888888",fontWeight:700,marginTop:4}}>📊 الشرائح — توزيع الدخل</div>
              {(budgetSettings.tranches||[]).map((tr)=>{
                const total=Object.values(tr.pcts).reduce((s,v)=>s+(v||0),0);
                return(
                  <div key={tr.id} style={{background:"rgba(255,255,255,.06)",borderRadius:12,padding:14,border:`1px solid ${total===100?"#10b98144":"#ef444444"}`}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                      <span style={{fontWeight:700,color:"#f5f5f0",fontSize:13}}>شريحة {tr.id}</span>
                      <span style={{fontSize:11,color:total===100?"#10b981":"#ef4444",fontWeight:700}}>{total}% {total===100?"✅":"⚠️"}</span>
                    </div>
                    <div style={{display:"flex",gap:6,marginBottom:8}}>
                      <div style={{flex:1}}>
                        <div style={{fontSize:9,color:"#666666",marginBottom:3}}>من (درهم)</div>
                      ...
                      <div style={{flex:1}}>
                        <div style={{fontSize:9,color:"#666666",marginBottom:3}}>إلى (درهم)</div>
                      ...
                      <div style={{flex:1}}>
                        <div style={{fontSize:9,color:"#666666",marginBottom:3}}>فيكس (درهم)</div>
              ...
              {/* ربط الحسابات */}
              <div style={{fontSize:12,color:"#888888",fontWeight:700,marginTop:4}}>🏦 ربط الحسابات</div>
              {(budgetSettings.allocations||[]).map(a=>{
                const takenKeys=(budgetSettings.allocations||[]).filter(x=>x.id!==a.id).flatMap(x=>x.accountKeys||[]);
                const availableAccs=allAcc.filter(ac=>!takenKeys.includes(ac.key));
                return(
                  <div key={a.id} style={{background:"white",borderRadius:12,padding:10,border:`2px solid ${a.color}44`}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                      <span style={{fontSize:18}}>{a.icon}</span>
                      <span style={{fontWeight:700,color:"#f5f5f0",fontSize:13}}>{a.name}</span>
                    </div>
                    {a.type==="expenses"&&<>
                      <div style={{display:"flex",gap:6,marginBottom:6}}>
                        <div style={{flex:1}}>
                          <div style={{fontSize:9,color:"#666666",marginBottom:3}}>تنبيه عند (درهم)</div>
                          <input style={{...S.inp,padding:"6px 8px",fontSize:12}} type="number" value={a.minAlert||300}
                            onChange={e=>setBudgetSettings(p=>({...p,allocations:p.allocations.map(x=>x.id===a.id?{...x,minAlert:parseFloat(e.target.value)||300}:x)}))}/>
                        </div>
                        <div style={{flex:1}}>
                          <div style={{fontSize:9,color:"#666666",marginBottom:3}}>تحويل من الطوارئ</div>
                          <input style={{...S.inp,padding:"6px 8px",fontSize:12}} type="number" value={a.emergencyTransfer||0}
                            onChange={e=>setBudgetSettings(p=>({...p,allocations:p.allocations.map(x=>x.id===a.id?{...x,emergencyTransfer:parseFloat(e.target.value)||0}:x)}))}/>
                        </div>
                      </div>
                    ...
                    {(a.accountKeys||[]).map(key=>{
                      ...
                      return acc?(
                        <div key={key} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",background:a.color+"15",borderRadius:8,marginBottom:5}}>
                          <span style={{flex:1,fontSize:11,color:"#f5f5f0"}}>{acc.bn} — {acc.name}</span>
            ...
            {modal==="dep"&&<div style={S.col}>
              {selSv&&<div style={{padding:12,background:"#f8fafc",borderRadius:10,display:"flex",gap:10,alignItems:"center"}}><span style={{fontSize:24}}>{selSv.icon}</span><div><div style={{fontWeight:700}}>{selSv.name}</div><div style={{fontSize:12,color:"#888888"}}>متبقي: {fmt(selSv.target-selSv.saved)}</div></div></div>}
              <input style={S.inp} placeholder="المبلغ" type="number" value={form.amount||""} onChange={e=>F("amount",e.target.value)}/>
        ...
        <div className="mwp" onClick={()=>setCd(null)}>
          <div className="mbx" style={{padding:28}} onClick={e=>e.stopPropagation()}>
            <div style={{textAlign:"center",marginBottom:20}}>
              ...
              <div style={{fontSize:14,color:"#666666"}}>هل تبغي تحذف <strong style={{color:"#1a1a1a"}}>"{cd.lbl}"</strong>؟</div>
              <div style={{fontSize:12,color:"#ef4444",marginTop:6}}>هذا الإجراء لا يمكن التراجع عنه</div>
            </div>
            <div style={{display:"flex",gap:10}}>
              <button style={{...S.btn("#e8e8e4"),border:"1px solid #334155",color:"#666666"}} onClick={()=>setCd(null)}>إلغاء</button>
          ...
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:500,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
            <div style={{background:"white",borderRadius:"24px 24px 0 0",width:"100%",maxWidth:480,padding:20,maxHeight:"90vh",overflowY:"auto"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                <span style={{fontSize:17,fontWeight:900,color:"#1a1a1a"}}>💰 توزيع الدخل</span>
                <button onClick={()=>setDistModal(null)} style={{background:"#f5f5f0",border:"none",borderRadius:10,padding:"6px 12px",cursor:"pointer",fontSize:13,color:"#666666"}}>تخطي</button>
              </div>
              {/* Steps dots */}
              <div style={{display:"flex",justifyContent:"center",gap:6,marginBottom:20}}>
                {steps.map((s,i)=>(
                  <div key={i} style={{width:distModal.step===i+1?24:8,height:8,borderRadius:4,background:i+1<=distModal.step?"#10b981":"#e8e8e4",transition:"all .3s"}}/>
                ))}
              </div>
              ...
              {distModal.step===1&&<>
                <div style={{textAlign:"center",padding:"16px 0"}}>
                  ...
                  <div style={{fontSize:14,color:"#666666",marginBottom:4}}>دخل جديد</div>
                  <div style={{fontSize:32,fontWeight:900,color:"#1a6b4a"}}>{fmt2(inc)} <span style={{fontSize:16}}>د.م</span></div>
                </div>
                <div style={{background:"#f8fafc",borderRadius:14,padding:14,marginBottom:16}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:13}}>
                    <span style={{color:"#666666"}}>الحد الثابت للمصاريف</span>
                    <span style={{fontWeight:700,color:"#1a1a1a"}}>{fmt2(Math.min(inc,thr))} د.م</span>
                  </div>
                  {surplus>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginTop:8}}>
                    <span style={{color:"#666666"}}>الفائض للتوزيع</span>
              ...
              {distModal.step===2&&<>
                <div style={{fontSize:13,color:"#666666",marginBottom:12}}>عدل النسب حسب رغبتك</div>
                {BKTS.map(b=>(
                  <div key={b.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:"1px solid #f1f5f9"}}>
                    <div style={{width:40,height:40,borderRadius:12,background:b.bg||"#f5f5f0",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{b.icon}</div>
                    <div style={{flex:1}}>
                      <div style={{fontSize:13,fontWeight:700,color:"#1a1a1a"}}>{b.name}</div>
                      <div style={{fontSize:11,color:"#888888"}}>{b.fixed?"الحد الثابت + "+expPct+"%":fmt2(calcAmt(b.id))+" د.م"}</div>
                    </div>
                ...
                <div style={{display:"flex",gap:8,marginTop:12}}>
                  <button style={{...S.btn("#e8e8e4"),flex:1,padding:12,color:"#666666"}} onClick={()=>setDistModal(p=>({...p,step:1}))}>← رجوع</button>
              ...
              {/* Step 3: Preview */}
              {distModal.step===3&&<>
                <div style={{fontSize:13,color:"#666666",marginBottom:12}}>معاينة التوزيع</div>
                {BKTS.map(b=>{
                  const amt=calcAmt(b.id);
                  const pct=inc>0?(amt/inc*100):0;
                  return(
                    <div key={b.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:"1px solid #f1f5f9"}}>
                      <div style={{width:40,height:40,borderRadius:12,background:b.bg||"#f5f5f0",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{b.icon}</div>
                      <div style={{flex:1}}>
                        <div style={{fontSize:13,fontWeight:700}}>{b.name}</div>
                        <div style={{height:4,background:"#f5f5f0",borderRadius:4,marginTop:4,overflow:"hidden"}}>
                ...
                <div style={{display:"flex",gap:8,marginTop:12}}>
                  <button style={{...S.btn("#e8e8e4"),flex:1,padding:12,color:"#666666"}} onClick={()=>setDistModal(p=>({...p,step:2}))}>← رجوع</button>
              ...
              {/* Step 4: Confirm */}
              {distModal.step===4&&<>
                <div style={{fontSize:13,color:"#666666",marginBottom:12}}>تأكيد التحويلات</div>
                {BKTS.map(b=>{
                  const amt=calcAmt(b.id);
                  ...
                  return(
                    <div key={b.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:b.bg||"#f8fafc",borderRadius:12,marginBottom:8}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{fontSize:13,fontWeight:700,color:"#1a1a1a"}}>{b.name}</div>
                        <div style={{fontSize:11,color:"#666666"}}>{(b.accountKeys||[]).length>0?"→ "+b.name:"ما كاين حساب مربوط"}</div>
                      </div>
                      <div style={{fontSize:15,fontWeight:900,color:b.color}}>{fmt2(amt)}</div>
                    </div>
                  );
                ...
                <div style={{display:"flex",gap:8,marginTop:12}}>
                  <button style={{...S.btn("#e8e8e4"),flex:1,padding:12,color:"#666666"}} onClick={()=>setDistModal(p=>({...p,step:3}))}>← رجوع</button>
              ...
              {distModal.step===5&&<>
                <div style={{textAlign:"center",padding:"16px 0"}}>
                  ...
                  <div style={{fontSize:18,fontWeight:900,color:"#1a1a1a",marginBottom:4}}>تم التوزيع!</div>
                  <div style={{fontSize:13,color:"#666666"}}>{fmt2(inc)} د.م تقسمو على {BKTS.length} حسابات</div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
                  {BKTS.map(b=>(
                    <div key={b.id} style={{background:b.bg||"#f8fafc",borderRadius:14,padding:12,textAlign:"center"}}>
                      <div style={{fontSize:20,marginBottom:4}}>{b.icon}</div>
                      <div style={{fontSize:10,color:"#666666",marginBottom:2}}>{b.name}</div>
